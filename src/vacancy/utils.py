from src.api.engine import HeadHunter, SuperJob
from src.saver.saver import ReadWriteToJSON
from src.vacancy.vacancy import VacanciesHH, VacanciesSJ, VacanciesSort
...
def sort_all():
    ...
    for i in all_vacancies:
        total_vacancies.append(VacanciesSort(i['url'], i['title'], i['city'], i['salary_int'],
                                             i['salary'], i['requirements'], i['date']))
...
class WorkToUser:
    ...
    def choice_site(self):
        ...
        while True:
            try:
                choice_user = int(
                    input(f'1 - {site_list[0]}\n2 - {site_list[1]}\nОпределите платформу для поиска вакансий: '))
